...
- 挂载脚本：**`ASSETS/erp-nav-rail-figma.js`**，由 **`demos/channel-transfer.html`** 等页面引用；`DOMContentLoaded` 时执行 `mountErpNavRailFromFigma()` 写入 `#erpNavSidebar`，并按悬停切换 `default` / `hover` / `active` 图标。HTML 不在 **`demos/`** 时须在引入脚本前设置 **`window.ERP_NAV_ASSET_BASE`**（见脚本文件头与 **`ASSETS/navigation/README.md`**、**`case-studies/good-case.md`** 索引）。
